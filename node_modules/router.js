	...
	if(!Object.keys(query).length>0){
		...
		switch(true){
			...
			case path.indexOf("-")>0||numbersOnly.test(path.split("/")[1]) :
				...
				if(path.split("/")[2]) {
					fs.readFile("html/"+serial+"/info.json", "utf8", function(err,data){
						if(err){
							throw err;
						} 
						else {
							...
							console.log(info);
							if(info.versions.indexOf(path.split("/")[2])>-1)	{
		...
		switch (path){
			...
			case "/get-text":
				...
				break;
			case "/get-comments":
				fs.readFile("./html/"+query.serial+"/"+query.vers+"/comments/"+query.issue+".txt", "utf8", function(err,data){
					if(err){
						res.write(err);
					}
					else{
				...
				break;
			case "/get-isuue":
				fs.readFile("./html/"+query.serial+"/"+query.vers+"/issues/"+query.issue+".json", "utf8", function(err,data){
					if(err){
						res.write(err);
					}
					else{
				...
				break;
			case "/get-issue-list":
				fs.readFile("./html/"+query.serial+"/"+query.vers+"/issues-list.json", "utf8", function(err,data){
					if(err){
						res.write(err);
					}
					else{
				...
				break;
			//newcomer processing for existing work
			case "/add-user":
				addUser.do(query.user,query.serial);
				break;
			//creates for in-work process (needed for sync)
			case "/create-issue":
				var issue={
					range:query.range,
					changeText:query.changeText,
					color:query.color
				}
				fs.writeFile("./html/"+query.serial+"/"+query.vers+"/issues/"+query.issueId+".json",JSON.stringify(issue));
				fs.appendFile("./html/"+query.serial+"/"+query.vers+"/issues-list.txt");
				res.write("ok");
				res.end();
				break;
			default:
